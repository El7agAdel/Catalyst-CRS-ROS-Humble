...
def generate_launch_description():
    ...
    run_pnp_loop = LaunchConfiguration("run_pnp_loop")
...
# ---- Gazebo Fortress (ros_gz_sim)
    ...
    delayed_moveit = TimerAction(period=2.5, actions=[move_group])

    pnp_pose_loop = ExecuteProcess(
        cmd=[
            "python3",
            "-m",
            "Catalyst.send_trajectory_pnp",
            "--ros-args",
            "-p",
            ["use_sim_time:=", use_sim_time],
        ],
    ...
    delayed_pnp_pose_loop = TimerAction(
        period=6.0,
        actions=[pnp_pose_loop],
        condition=IfCondition(run_pnp_loop),
    )
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "run_pnp_loop",
                default_value="true",
                description="Run continuous end-effector pose A/B loop",
            ),
